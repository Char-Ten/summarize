...
RPA.setDistanceFn(function(p1, p2) {
    /**
     * 获取两个经纬度坐标的距离
     * 高德地图的写法为 new AMap.LngLat(p1.lat, p1.lng).distance([p2.lat, p2.lng]);
     * 百度地图的写法为 map.getDistance(new BMap.Point(p1.lat, p1.lng),new BMap.Point(p2.lat, p2.lng));
     */
    return L.latLng(p1.lat, p1.lng).distanceTo(L.latLng(p2.lat, p2.lng))
});
var a = RPA.setOptions({
    polygon: [{
        "lat": 23.172429366766064,
        "lng": 113.40926885604858
    }, {
        "lat": 23.17182770502691,
        "lng": 113.4084963798523
    }, {
        "lat": 23.171107680046834,
        "lng": 113.4082818031311
    }, {
        "lat": 23.17075259780888,
        "lng": 113.40996623039244
    }, {
        "lat": 23.17160084825929,
        "lng": 113.41047048568726
    }],
    stepRotate: 0,
    space: 5
});